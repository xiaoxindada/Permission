package com.xiaoxin.permissionmanager;
...
public class Permission {
     ...
     * @return 权限是否已申请成功的结果
     */
    public static boolean isGrated(Context context, String permission, boolean verbose) {
        final String TAG = "PermissionCheck";
        if (ContextCompat.checkSelfPermission(context, permission) != PackageManager.PERMISSION_GRANTED) {
            if (verbose) Toast.makeText(context,"permission: " + permission + " died", Toast.LENGTH_LONG).show();
            if (verbose) Log.i(TAG, "permission: " + permission + " died");
            return false;
        } else {
            return true;
        }
    ...
    public static boolean AllIsGrated(Context context, String[] permissions, boolean verbose) {
        ...
        if (diedList.size() > 0) {
            if (verbose) Log.i(TAG, "PermissionDiedList: " + diedList);
            if (verbose) Toast.makeText(context,"PermissionDiedList: " + diedList, Toast.LENGTH_LONG).show();
            return false;
        } else {
            return true;
        }
    ...
    *
    **/
    public static void forwardToSettings(Context context) {
